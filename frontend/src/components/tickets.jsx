        ...
        <div className="p-6">
          <div className="flex flex-col items-start sm:flex-row sm:items-center text-left ">
            <div>
              <p className="text-gray-400">
                ...
                {props.username} - โพสต์ {props.timeCreated.substring(11, 16)} น.
              </p>
          ...
          <div className="flex flex-wrap justify-between">
            <p className="inline-flex items-center">
              ...
              {/* <span className="text-gray-400 bg-red-500"> {props.status}
                  {props.status === "กำลังรับออเดอร์" ? props.status : "อื่นๆ"}
                  </span> */}
              <span
                className={`text-white rounded-lg  bg-${
                  props.status === "กำลังรับออเดอร์"
                    ? "emerald"
                    : props.status === "ส่งเรียบร้อย"
                    ? "gray"
                    : props.status === "ถึงจุดนัดแล้ว"
                    ? "amber"
                    : "red"
                }-500`}
              >
                <p className="px-2">{props.status}</p>
              </span>
            </p>
          </div>
        </div>
